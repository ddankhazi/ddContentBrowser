...
import os as _os
print("=" * 60)
print("DEBUG: batch_rename.py is being loaded/reloaded!")
print(f"DEBUG: File path: {_os.path.abspath(__file__)}")
print("=" * 60)
...
class BatchRenameDialog(QDialog):
    ...
    def create_options_panel(self):
        ...
        # DEBUG: Print available modes
        print("DEBUG: Available modes in combo box:")
    ...
    def on_mode_changed(self, mode):
        ...
        if mode == "Find & Replace":
            ...
            print("DEBUG: Calling setup_multistep_options()...")
            self.setup_multistep_options()
        else:
            print(f"DEBUG: WARNING - Unknown mode: '{mode}'")
    ...
    def setup_multistep_options(self):
        """Setup Multi-Step options with 3 sequential operations"""
        print("DEBUG: setup_multistep_options() called!")
        
        help_label = QLabel("Apply 2-3 operations in sequence. Output of each step becomes input for the next.")
        help_label.setWordWrap(True)
        help_label.setStyleSheet("color: #888; font-size: 10px; padding: 5px;")
        self.options_area.addWidget(help_label)
        
        print("DEBUG: Creating Step 1 combo box...")
        # Available operations (excluding Multi-Step itself)
        operations = [
            "None",
            "Find & Replace",
            "New Name",
        ...
        step1_layout.addWidget(QLabel("Step 1:"))
        self.step1_combo = QComboBox()
        self.step1_combo.addItems(operations)
        self.step1_combo.setCurrentText("None")
        self.step1_combo.currentTextChanged.connect(self.update_preview)
        step1_layout.addWidget(self.step1_combo)
        self.options_area.addLayout(step1_layout)
        
        # Step 2
        step2_layout = QHBoxLayout()
        step2_layout.addWidget(QLabel("Step 2:"))
        self.step2_combo = QComboBox()
        self.step2_combo.addItems(operations)
        self.step2_combo.setCurrentText("None")
        self.step2_combo.currentTextChanged.connect(self.update_preview)
        step2_layout.addWidget(self.step2_combo)
        self.options_area.addLayout(step2_layout)
        
        # Step 3
        step3_layout = QHBoxLayout()
        step3_layout.addWidget(QLabel("Step 3:"))
        self.step3_combo = QComboBox()
        self.step3_combo.addItems(operations)
        self.step3_combo.setCurrentText("None")
        self.step3_combo.currentTextChanged.connect(self.update_preview)
        step3_layout.addWidget(self.step3_combo)
        self.options_area.addLayout(step3_layout)
        
        # Note about how to use it
        note_label = QLabel("Note: Configure each step's settings by temporarily switching to that mode.")
        note_label.setWordWrap(True)
        note_label.setStyleSheet("color: #CC8800; font-size: 9px; padding: 5px; font-style: italic;")
        self.options_area.addWidget(note_label)
    ...
    def update_preview(self):
        ...
        try:
            if mode == "Find & Replace":
                ...
                self.preview_names = self.apply_multistep(preview=True)
    ...
    def apply_multistep(self, preview=False):
        """Apply multiple rename operations in sequence"""
        if not hasattr(self, 'step1_combo'):
            return self.original_names.copy()
        
        # Temporarily store original names
        temp_original_names = self.original_names.copy()
        
        # Get enabled steps
        steps = []
        for combo in [self.step1_combo, self.step2_combo, self.step3_combo]:
            step_mode = combo.currentText()
            if step_mode != "None":
                steps.append(step_mode)
        ...
        for step_mode in steps:
            # Update original_names temporarily for this step
            self.original_names = current_names
            
            # Apply the step based on mode
            if step_mode == "Find & Replace":
                current_names = self.apply_find_replace(preview=True)
            elif step_mode == "New Name":
                current_names = self.apply_new_name(preview=True)
            elif step_mode == "Add Prefix":
                current_names = self.apply_prefix(preview=True)
            elif step_mode == "Add Suffix":
                current_names = self.apply_suffix(preview=True)
            elif step_mode == "Numbering":
                current_names = self.apply_numbering(preview=True)
            elif step_mode == "Case Change":
                current_names = self.apply_case_change(preview=True)
            elif step_mode == "Regex Pattern":
                current_names = self.apply_regex(preview=True)
        
        # Restore original names
        self.original_names = temp_original_names
